...
def _make_semantic_physical_drift_context_diff(
    snapshot: Snapshot,
    *,
    ...
    return ContextDiff(
        ...
        added=set(),
        removed_snapshots={},
        modified_snapshots=modified_snapshots or {},
        snapshots={snapshot.snapshot_id: snapshot},
        new_snapshots=new_snapshots or {},
...
def test_inject_semantic_physical_drift_marks_partition_key_column_mismatch(
    ...
    adapter.get_live_semantic_key_property.return_value = (
        "unique_key",
        exp.Tuple(expressions=[exp.column("ds"), exp.column("store_id"), exp.column("goods_id")]),
    )
    ...
    assert old_snapshot.model.physical_properties["unique_key"] == exp.Tuple(
        expressions=[exp.column("ds"), exp.column("store_id"), exp.column("goods_id")]
    )
    assert context_diff.new_snapshots[snapshot.snapshot_id] is snapshot
    assert snapshot.change_category is None


def test_inject_semantic_physical_drift_skips_matching_key(
    mocker: MockerFixture,
) -> None:
    model = SqlModel(
        name="test_model",
...
def test_inject_semantic_physical_drift_updates_existing_modified_snapshot(
    ...
    previous_model = SqlModel(
        name="test_model",
        query=parse_one("select 1 as id, 2 as goods_id"),
        kind=dict(name=ModelKindName.INCREMENTAL_BY_UNIQUE_KEY, unique_key="id"),
        dialect="doris",
        physical_properties=d.parse_one("(unique_key = (id, goods_id))"),
    )
    ...
    previous_snapshot = Snapshot.from_node(previous_model, nodes={previous_model.fqn: previous_model})
    current_snapshot = Snapshot.from_node(current_model, nodes={current_model.fqn: current_model})
    context_diff = _make_semantic_physical_drift_context_diff(
        current_snapshot,
        modified_snapshots={current_snapshot.name: (current_snapshot, previous_snapshot)},
        new_snapshots={current_snapshot.snapshot_id: current_snapshot},
    )

    adapter = mocker.Mock()
    adapter.get_live_semantic_key_property.return_value = (
        "unique_key",
        exp.Tuple(expressions=[exp.column("id"), exp.column("goods_id")]),
    )
    stub_context = SimpleNamespace(engine_adapters={"": adapter}, engine_adapter=adapter)
    ...
    new_snapshot, old_snapshot = context_diff.modified_snapshots[current_snapshot.name]
    assert new_snapshot is current_snapshot
    assert old_snapshot is not previous_snapshot
    assert old_snapshot.model.physical_properties["unique_key"] == exp.Tuple(
        expressions=[exp.column("id"), exp.column("goods_id")]
    )
    assert context_diff.new_snapshots[current_snapshot.snapshot_id] is current_snapshot
